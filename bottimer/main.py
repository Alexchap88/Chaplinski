...
def start(message):
    bot.send_message(message.chat.id, 'Привет! Я бот, который устанавливает таймер.', reply_markup=get_keyboard())
...
def pre_set_text(message, times):
    bot.send_message(message.chat.id, 'Введите текст, который придёт после истечения таймера.')
...
def set_text(message, times):
    ...
    bot.send_message(message.chat.id, 'Спасибо! Через заданное время вам придёт уведомление.')
